...
inline DVal operator+(const DVal& x, const DVal& y)
{
	DVal dval(x.m);
	for (uint32_t i = 0; i < dval.n(); ++i)
...
inline DVal operator+(const DVal& x, const double& c)
{
	DVal dval(x);
	dval.d[0] += c;
	return dval;
...
inline DVal operator+(const double& c, const DVal& x)
{
	DVal dval(x);
	dval.d[0] += c;
	return dval;
...
namespace std
{
	...
	inline DVal log10(const DVal& x)
	{
		DVal dval(x.m);
	...
	// x.d[0] > 0
	inline DVal pow(const DVal& x, const double& n)
	{
		DVal dval(x.m);
	...
	// a > 0.0
	inline DVal pow(const double& a, const DVal& x)
	{
		DVal dval(x.m);
	...
	inline ostream& operator<<(std::ostream& s, const DVal& dval)
	{
		s << dval.d[0] << " (";
		for (uint32_t i = 1; i < dval.n(); ++i)
		{
