...
namespace bfgs::var
{
	// Version with dynamic dimension.
	class BFGS
	{
	private:
		...
		inline double _f_info(const std::function<double (double*, uint32_t)>& f, double* const x, double* const g)
		{
			return f(x, _n);
		}

		inline double _f_info(const std::function<double (double*, double*, uint32_t)>& f, double* const x, double* const g)
		{
			return f(x, g, _n);
		}

		inline double _f_info(const std::function<DVal (DVal*, uint32_t)>& f, double* const x, double* const g)
		{
			for (uint32_t i = 0; i < _n; ++i)
				_dval[i].set(x[i], i);
			DVal r = f(_dval, _n);
		...
		inline void _f_grad(const std::function<double (double*, uint32_t)>& f, const double& y, double* const x, double* const g)
		{
			if (_central_diff)
			{
				for (uint32_t i = 0; i < _n; ++i)
		...
		inline void _f_grad(const std::function<double (double*, double*, uint32_t)>& f, const double& y, double* const x, double* const g)
		{
		}

		inline void _f_grad(const std::function<DVal (DVal*, uint32_t)>& f, const double& y, double* const x, double* const g)
		{
		}
